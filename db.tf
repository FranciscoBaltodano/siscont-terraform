...
resource "azurerm_private_endpoint" "sql-private_end" {
    name                = "sql_private_endpoint_${var.project}-${var.environment}"
    resource_group_name = azurerm_resource_group.rg.name
    location            = var.location
...
resource "azurerm_private_dns_a_record" "private_dns_a_record" {
    ...
    resource_group_name = azurerm_resource_group.rg.name  
    ttl                 = 300
    records             = [azurerm_private_endpoint.sql-private_end.private_service_connection[0].private_ip_address] 
}
